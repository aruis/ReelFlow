...
struct AdvancedSettingsPanel: View {
    ...
    var body: some View {
        Form {
            Section("导出设置") {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Slider(value: transitionDurationBinding, in: RenderEditorConfig.transitionDurationRange, step: 0.05)
                        .disabled(isControlDisabled || !viewModel.config.enableCrossfade)
            ...
            Section("高级布局") {
                ...
                Toggle("显示底部铭牌文字", isOn: plateEnabledBinding)
                    .disabled(isControlDisabled)
                Picker("信息位置", selection: platePlacementBinding) {
                    Text("相框").tag(PlatePlacement.frame)
                    Text("黑底下方").tag(PlatePlacement.canvasBottom)
                }
                .disabled(isControlDisabled || !viewModel.config.plateEnabled)
        ...
        .formStyle(.grouped)
        .onAppear(perform: activateCustomPlateEditorIfNeeded)
    }
    ...
    private var plateContentEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            if viewModel.config.plateEnabled {
                Picker("字体风格", selection: $viewModel.config.plateFontStyle) {
    ...
    private var platePlacementBinding: Binding<PlatePlacement> {
        Binding(
            get: { viewModel.config.platePlacement },
            set: { placement in
                activateCustomPlateEditorIfNeeded()
                viewModel.config.platePlacement = placement
            }
        )
    }
